...
export default function SigningForm({
  ...
  return (
    <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[1.05fr_0.95fr]">
      <section className="animate-rise rounded-2xl border border-[var(--ws-border)] bg-[var(--ws-card)] p-4 sm:p-5">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <h2 className="text-sm font-semibold uppercase tracking-[0.14em] text-[var(--ws-accent)]">
            Agreement
          </h2>
          {documents.length > 1 && (
              ...
              {documents.map((doc, index) => (
                <button
                  ...
                  className={`rounded-lg px-3 py-1 text-xs font-semibold ${
                    index === activeDoc
                      ? 'bg-[var(--ws-btn)] text-[var(--ws-page)]'
                      : 'border border-[var(--ws-border)] text-[var(--ws-body)]'
                  }`}
                >
                  {doc.name}
                </button>
        ...
        {currentDoc ? (
          <div className="overflow-hidden rounded-xl border border-[var(--ws-border)] bg-white">
            <iframe title={currentDoc.name} src={currentDoc.url} className="h-[70vh] w-full" />
          ...
          <div className="flex h-64 items-center justify-center rounded-xl border border-dashed border-[var(--ws-border)] text-sm text-[var(--ws-dim)]">
            Review the fields on the right, then sign to complete this agreement.
          </div>
        )}
      </section>

      <section className="animate-rise delay-1 space-y-5 rounded-2xl border border-[var(--ws-border)] bg-[var(--ws-card)] p-5 sm:p-6">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--ws-body)]">Signing as {recipientName}</p>
        </div>
        ...
        >
          <div className="space-y-3">
            <h3 className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--ws-dim)]">
              Your details
            </h3>
            {editableFields.map((field) => {
              if (field.kind === 'checkbox') {
                return (
                  <label key={field.name} className="flex items-start gap-3 text-sm text-white">
                    <input
                      type="checkbox"
                      className="mt-1 h-4 w-4 rounded border-[var(--ws-border)] bg-[var(--ws-panel)]"
              ...
              return (
                <label key={field.name} className="block">
                  <span className="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--ws-dim)]">
                    {field.name}
                    {field.required ? ' *' : ''}
                    {field.uuids.length > 1 ? (
                      <span className="ml-1 font-normal normal-case tracking-normal text-[var(--ws-dim)]">
          ...
          <div>
            <div className="mb-2 flex items-center justify-between gap-2">
              ...
              <button
                ...
                className="text-xs text-[var(--ws-dim)] hover:text-white"
              >
                Clear
              </button>
            </div>
            <canvas
              ref={canvasRef}
              className="h-36 w-full touch-none rounded-xl border border-[var(--ws-border)] bg-white"
              onPointerDown={(e) => {
                const canvas = canvasRef.current;
                const ctx = canvas?.getContext('2d');
            ...
            />
            <p className="mt-1.5 text-xs text-[var(--ws-dim)]">Draw your signature with mouse or finger.</p>
          </div>
