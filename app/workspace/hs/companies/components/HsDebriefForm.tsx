...
import { saveHsDebriefAction } from '@/lib/workspace/hs-company-actions'
import {
  HS_DEBRIEF_OUTCOMES,
  HS_DEBRIEF_TIMELINES,
  HS_OBJECTION_CHIPS,
  HS_SEASON_TIMINGS,
  formatNumberInput,
  hsDebriefDraftFrom,
  hsDebriefOutcomeLabel,
  hsDebriefTimelineLabel,
  hsSeasonTimingLabel,
  parseNumberInput,
  type HsDebriefDraft,
  type HsDebriefOutcome,
  type HsDebriefRecord,
  type HsDebriefTimeline,
  type HsSeasonTiming,
...
type FormValues = {
  outcome: HsDebriefOutcome | ''
  biggest_leak: string
  their_words: string
  objections_raised: string[]
  objection_notes: string
  decision_makers: string
  season_timing: HsSeasonTiming | ''
  timeline: HsDebriefTimeline | ''
...
function valuesFrom(debrief: HsDebriefRecord | null): FormValues {
  const draft = hsDebriefDraftFrom(debrief)
  return {
    ...
    season_timing: draft.season_timing ?? '',
...
function toDraft(values: FormValues): HsDebriefDraft {
  return {
    ...
    season_timing: values.season_timing || null,
...
export default function HsDebriefForm({
  companyId,
  debrief,
}: {
  companyId: string
  ...
  const persist = useCallback(
    async (applyOutcomeStage: boolean) => {
      const result = await saveHsDebriefAction(
        companyId,
    ...
    <div className="mx-auto w-full max-w-[720px] space-y-6">
      <div>
        ...
        <div className="space-y-2" role="radiogroup" aria-label="Outcome">
          {HS_DEBRIEF_OUTCOMES.map((outcome) => {
      ...
      <div>
        <p className="mb-1.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-neutral-500">
          Where they are in their season
        </p>
        <p className="mb-2 text-xs text-[#6E6C80]">
          Seasonality changes when they can start and what they will pay
        </p>
        <div className="flex flex-wrap gap-2" role="radiogroup" aria-label="Where they are in their season">
          {HS_SEASON_TIMINGS.map((timing) => {
            const active = values.season_timing === timing
            return (
              <button
                key={timing}
                type="button"
                role="radio"
                aria-checked={active}
                onClick={() => patch('season_timing', timing)}
                className="rounded-full border px-3 py-1.5 text-[12px] font-semibold"
                style={{
                  borderColor: active ? '#937DFF' : '#2A2A3A',
                  backgroundColor: active ? '#937DFF22' : 'transparent',
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2" role="radiogroup" aria-label="Timeline">
          {HS_DEBRIEF_TIMELINES.map((timeline) => {
            ...
            return (
              ...
              >
                {hsDebriefTimelineLabel(timeline)}
              </button>
            )
          })}
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Due">
          <Input type="date" value={values.next_step_due} onChange={(e) => patch('next_step_due', e.target.value)} />
        </Field>
      </div>
