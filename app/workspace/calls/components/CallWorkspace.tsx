...
import {
  generateCallCalendarLinkAction,
  markCallCalendarSentAction,
  saveCallAnswerAction,
  saveCallNotesAction,
  saveStageProgressAction,
  setCallOutcomeAction,
  updateCallStatusAction,
} from '@/lib/workspace/call-actions';
import {
  CALL_STAGES,
  CALL_STATUSES,
  STAGE_PROGRESS_KEY,
...
function ClickToCopy({ value, empty = '—' }: { value: string | null; empty?: string }) {
  ...
  if (!value) return <span className="text-[var(--ws-dim)]">{empty}</span>;
  return (
    <button
      type="button"
...
function answersMap(rows: CallAnswer[]): Record<string, { text: string; flagged: boolean }> {
  const map: Record<string, { text: string; flagged: boolean }> = {};
  for (const row of rows) {
    if (row.question_key === STAGE_PROGRESS_KEY) continue;
...
export default function CallWorkspace({
  ...
  const [openStage, setOpenStage] = useState<CallStageId>(progress.open);
  const [completed, setCompleted] = useState<CallStageId[]>(progress.completed);
  ...
  const persistProgress = useCallback(
    (nextOpen: CallStageId, nextCompleted: CallStageId[]) => {
      startTransition(async () => {
        await saveStageProgressAction(call.id, nextOpen, nextCompleted);
      });
    },
    [call.id],
  );
  ...
  useDebounced(notes, 2000, (value) => {
    startTransition(async () => {
      await saveCallNotesAction(call.id, value);
    });
  }, true);
  ...
  function open(stage: CallStageId) {
    setOpenStage(stage);
    persistProgress(stage, completed);
  }

  function markComplete(stage: CallStageId) {
    const nextCompleted = completed.includes(stage) ? completed : [...completed, stage];
    const nextOpen = nextStageId(stage) ?? stage;
      ...
      <header className="sticky top-16 z-20 flex flex-wrap items-center gap-3 border-b border-white/10 bg-[#0B0B0F]/90 px-4 py-3 backdrop-blur-xl lg:top-0 lg:px-6">
        ...
        <Select
          ...
          onChange={(e) => {
            ...
            setStatus(next);
            startTransition(async () => {
              await updateCallStatusAction(call.id, next);
            });
          }}
        >
      ...
      <div className="grid flex-1 grid-cols-1 gap-4 p-4 lg:grid-cols-[minmax(220px,300px)_minmax(0,1fr)_minmax(240px,320px)] lg:items-start lg:gap-5 lg:p-5">
        <section className="space-y-4">
          <div className={`${ws.card} p-4`}>
            ...
            <h2 className={headingClass('mt-2 text-lg font-semibold')}>{call.practice_name}</h2>
            <p className="mt-1 text-sm text-[#B0AEC0]">{practiceTypeLabel(call.practice_type)}</p>
            <p className="mt-4 text-sm text-white">
              {call.contact_name}
              <span className="text-[#6E6C80]"> · {callRoleLabel(call.role)}</span>
            </p>
            <p className={headingClass('mt-5 text-3xl font-semibold tabular-nums')}>
              {frontDeskLabel(call.front_desk_size)}
            </p>
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#6E6C80]">Front desk</p>
            {call.stated_pain ? (
                ...
                <div key={stage.id}>
                  ...
                  {isOpen && (
                    <div className="pb-5">
                      ...
                      <div className="mt-4">
                        <Button type="button" size="sm" onClick={() => markComplete(stage.id)}>
                          Mark complete
                        </Button>
                      </div>
                    </div>
              ...
              <>
                <Button
                  ...
                  onClick={() => {
                    setError(null);
                    startTransition(async () => {
                      const result = await generateCallCalendarLinkAction(call.id);
                ...
                <Button
                  ...
                  onClick={() => {
                    startTransition(async () => {
                      const result = await markCallCalendarSentAction(call.id);
                      if (!result.ok) setError(result.error);
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (!outcome) return;
              startTransition(async () => {
                const result = await setCallOutcomeAction(call.id, outcome);
                if (result && !result.ok) {
