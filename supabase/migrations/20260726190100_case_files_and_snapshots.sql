-- Vistrial: case files, the metric dictionary, and snapshots.
--
-- The baseline and every progress snapshot measure the same metric keys, which
-- is what makes them directly comparable. Metrics live in a dictionary table
-- rather than as columns so adding one later does not need a migration and does
-- not break comparability with older snapshots.
...
comment on table public.metric_definition is
  'The fields captured in the baseline and re-measured in every progress snapshot. `direction` decides whether a rise is an improvement, which is how the growth report can show what got worse without hand-labelling.';
...
create table public.client_case_file (
  ...
  -- Set by begin_install(). Its presence is what locks the baseline.
...
comment on table public.client_case_file is
  'Master record for one engagement. Every other object attaches to this.';
comment on column public.client_case_file.install_started_at is
  'Rule 1: once set, the baseline is locked and can never be edited, only annotated.';
...
-- Drive subfolders, one row per category. Vistrial stores the reference; Drive
-- holds the bytes.
...
create table public.snapshot (
  ...
  -- Null means still editable. A progress snapshot is locked the moment it is
  -- taken; a baseline is locked when the install begins.
...
comment on column public.snapshot.locked_at is
  'Rules 1 and 2. Once set, this snapshot and its metrics reject every write.';

-- One baseline per case file, enforced in the schema rather than in code.
create unique index snapshot_one_baseline_per_case_file
  on public.snapshot (case_file_id)
  where kind = 'baseline';

create index snapshot_case_file_taken_idx
  on public.snapshot (case_file_id, taken_at desc);
...
comment on column public.snapshot_metric.source is
  'Whether this figure is defensible or the client''s estimate.';

create index snapshot_metric_key_idx on public.snapshot_metric (metric_key);

-- Lead volume by source, where the client knows it.
...
-- A locked snapshot can be explained but never changed. This is how an anomaly
-- -- a fortnight with the ads paused -- gets recorded without touching numbers.
...
-- A snapshot row may only change to acquire its lock. Nothing else moves.
...
begin
  if old.locked_at is not null then
    raise exception 'snapshot_locked: snapshot % locked at % and can no longer be edited. Annotate it instead.',
  ...
  if new.id <> old.id
     ...
     or new.taken_at <> old.taken_at then
    raise exception 'snapshot_identity_fixed: a snapshot''s identity, case file, kind and timestamp cannot change'
      using errcode = '23514';
  end if;
...
begin
  if old.locked_at is not null then
    raise exception 'snapshot_locked: snapshot % is locked and cannot be deleted', old.id
      using errcode = '23514';
  end if;
  return old;
end;
...
-- Child rows follow the parent's lock.
...
begin
  ...
  if locked is not null then
    raise exception 'snapshot_locked: snapshot % locked at %; its measurements are immutable', target, locked
      using errcode = '23514';
  end if;
...
-- Annotations are themselves append-only.
create trigger snapshot_annotation_immutable
  before update or delete on public.snapshot_annotation
  for each row execute function app.forbid_mutation();

-- ---------------------------------------------------------------------------
-- GoHighLevel ingestion landing zone
-- ---------------------------------------------------------------------------

-- Daily tracking values as ingested. Automatic weekly snapshots roll this up,
-- which is the only reason they can be taken without a human present.
...
create index tracking_metric_daily_lookup_idx
  on public.tracking_metric_daily (case_file_id, metric_key, day desc);

comment on table public.tracking_metric_daily is
  'Landing zone for the GoHighLevel ingestion. Automatic snapshots aggregate from here.';
