...
| `50_operating_config.sql` | The industry templates are rows, a select carries its options, and neither a template nor a per-client override may shadow a locked core key. A client is configured by column, so editing its notes no longer changes what operators are asked. An override replaces the template rather than merging with it. Switching template keeps a qualified-booking definition the admin wrote themselves. An escalation is routed to people who exist. |
| `60_applications.sql` | An application lands from the public internet with no session, is normalised, and cannot be read back by `anon`. A second submission is a correction rather than a second application and does not wipe fields left blank. A nameless or unreachable application is refused, as is a burst from one address. Reviewing one is admin-only and audited. |
